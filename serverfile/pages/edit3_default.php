...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($_POST['action']) {
        ...
        case 'setDefault_item':
            if (isset($_POST['itemID'])) {
                $sql = "UPDATE `groups` SET `defaultItemID`= ? WHERE `groupName` = 'sitLimitFreq'";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("i", $_POST['itemID']);
                if ($stmt->execute()) {
                    echo json_encode([
        ...
        case 'add_item':
            if (isset($_POST['itemValue'])) {
                $groupID = 3; // Replace with the actual group ID
...
<style>
    ...
    <div class="mx-3">
        <div class="row mb-4">
            <div class="col-md-7">
                <label for="edtime2" class="form-label fs-4"><b>ความถี่ในการแจ้งเตือนเมื่อนั่งต่อเนื่องเกินระยะเวลาที่กำหนด :</b></label>
            </div>
