...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($_POST['action']) {
        ...
        case 'setDefault_item':
            if (isset($_POST['itemID'])) {
                $sql = "UPDATE `groups` SET `defaultItemID`= ? WHERE `groupName` = 'detectFreq'";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("i", $_POST['itemID']);
                if ($stmt->execute()) {
                    echo json_encode([
...
<style>
    ...
    <div class="mx-3">
        <div class="row mb-4">
            <div class="col-md-6">
                <label for="edtime2" class="form-label fs-4"><b>ความถี่การตรวจจับท่านั่ง :</b></label>
            </div>
            <div class="col-md-2 text-center">
                <input class="form-control text-center fs-4" min="1" type="number" id="addItemInput" value="">
            </div>
            <div class="col-md-1 mt-1">
                <span class="fs-4"><b>วินาที</b></span>
            </div>
        ...
        <div class="table-responsive w-100">
            <table class="table table-bordered table-hover tb1 w-100">
                <thead class="table">
                    <tr>
                        <th class="text-center bg-dark-subtle fs-4" style="width: 35%;">วินาที</th>
